...
def main():
    ...
    for i in range(args.count):
        ...
        # difference = 6 - len(random_str)
        # if difference != 0:
        #     padding = numpy.zeros(difference)
        #     for i in range(len(padding)):
        #         random_str = random_str + " "
        # random_str = [x for x in random_str]
        # random.shuffle(random_str)
        # shuffle_str = ''.join(random_str)
        # test = shuffle_str.replace(" ","")
        # if len(test)==0:
        #     continue;
        random_str = random_str.replace(" ","")
        if len(random_str)==0:
            continue
        if len(random_str) < args.length:
            continue
