...
def create_model(captcha_length, captcha_num_symbols, input_shape):
    ...
    x = input_tensor
    for i, module_length in enumerate([3] * 3):
        for j in range(module_length):
            x = keras.layers.Conv2D(filters=32, kernel_size=(3, 3), padding='same', kernel_initializer='he_uniform')(x)
...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            if not self.files.keys()==[]:
                random_image_label = random.choice(list(self.files.keys()))
                if random_image_label.startswith('.'):
                    print('######### going here for file:'+random_image_label)
...
def main():
    ...
    # createnewfiles(args.train_dataset, args.batch_size, args.length, captcha_symbols, args.width, args.height)
